...
describe('profile', () => {
  ...
  it('should not set a profile', async () => {
    ...
    try {
      ...
      expect(JSON.parse(error.message).error).toStrictEqual('missing "address" argument');
    }
    ...
    try {
      ...
      expect(JSON.parse(error.message).error).toStrictEqual('"theme" argument must be composed of 3 or 6 characters.');
    }
    ...
    try {
      ...
      expect(JSON.parse(error.message).error).toStrictEqual('"J" is not a valid hex character');
    }
  });
